...
use alloc::format;
use anyhow::Context;
use crate::netlink::utils_error::DecodeError;
    ...
    I: NetlinkDeserializable,
{
    fn parse(buf: &NetlinkBuffer<&'buffer B>) -> Result<Self, DecodeError> {
        ...
        let header =
            match <NetlinkHeader as Parseable<NetlinkBuffer<&'buffer B>>>::parse(buf) {
                Ok(header) => header,
                Err(_) => {
                    return Err(format!(
                        "failed to parse netlink header"
                    )
                    .into());
                }
            };
        ...
        let payload = match header.message_type {
            NLMSG_ERROR => {
                let msg = match ErrorBuffer::new_checked(&bytes)
                    .and_then(|buf| ErrorMessage::parse(&buf)){
                        Ok(msg) => msg,
                        Err(_) => {
                            return Err(format!(
                                "failed to parse NLMSG_ERROR"
                            ).into());
                        }
                    };
                Error(msg)
            ...
            NLMSG_NOOP => Noop,
            NLMSG_DONE => {
                let msg = match DoneBuffer::new_checked(&bytes)
                    .and_then(|buf| DoneMessage::parse(&buf)){
                        Ok(msg) => msg,
                        Err(_) => return Err(format!("failed to parse NLMSG_DONE").into()),
                    };
                Done(msg)
            }
            NLMSG_OVERRUN => Overrun(bytes.to_vec()),
            message_type => {
                let inner_msg = match I::deserialize(&header, bytes){
                    Ok(msg) => msg,
                    Err(_) => {
                        return Err(
                            format!("Failed to parse message with type {message_type}").into()
                        );
                    },
                };
